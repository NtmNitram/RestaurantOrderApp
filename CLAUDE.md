...
- **Tailwind v4 con `@tailwindcss/vite`:** no existe `tailwind.config.*` ni `tailwind.config.ts` en este proyecto. La detección de contenido es automática. El punto de entrada es `@import "tailwindcss"` en `index.css`. Para safelist de clases dinámicas (generadas en runtime), usar `@source inline("clase-a clase-b")` en `index.css` — no hay `safelist` en config. Antes de asumir que una clase está siendo purgada, verificar con build + grep en `dist/assets/*.css`.
- **Nunca hardcodear nombres de grupo de paquete en el frontend:** usar los flags `isCountingGroup`/`allowExtra` de `PackageGroupDto` para identificar el rol de un grupo (ej. pre-selección, validaciones). Los nombres como `'3er Tiempo'` o `'Guisados'` varían entre paquetes y tenants — un nombre hardcodeado silenciosamente falla con cualquier otro paquete. Aplica a todo componente que itere sobre `pkg.groups`.
- **Nombres de `MenuItem` y `PackageOption` son siempre dinámicos:** el frontend nunca hardcodea nombres de platillos o paquetes — los lee de la respuesta del backend (`nombreArticulo`, `optionNameSnapshot`, `pkg.name`). Cambiar el nombre de un ítem en la DB se refleja automáticamente en toda la UI sin tocar código.
